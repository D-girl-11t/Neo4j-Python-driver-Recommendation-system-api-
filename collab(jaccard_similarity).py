...
def choose(uk):
    if request.method=='POST':
        cor="""match (a:Users)-[:Likes_this]->(b)
with {item:id(a), name:a.title,categories:collect(id(b))} as Data
with collect(Data) as data
WITH data,[value in data WHERE value.name= $d | value.item ] AS sourceIds
call algo.similarity.jaccard.stream(data,{sourceIds: sourceIds,similarityCutoff: 0.0,topK: 10})
YIELD item1, item2, count1, count2, intersection, similarity
WITH algo.getNodeById(item1) AS from, algo.getNodeById(item2) AS to, similarity

with COLLECT(to) as neighbours
                                             
UNWIND neighbours as neighbour

match (neighbour)-[:POSTEDevent]->(z:Events)
return neighbour.title,collect(z.ename)"""
        print(uk[1:-1])
        print(type(uk))
        ww=uk[1:-1]
        
        conrec=k.Query(cor,{'d':ww})
